...
from landlab.components import BedloadTransport, OverlandFlowVarRoughnessRainfall
...
rainfallIntensityDEM = 'testChannelRainfall.asc'    # ASCII raster containing the rainfall intensity in mm/hr

dtPrecision = 3         # Avoids rounding errors
max_dt = 1             # Using only the default time step calculator gives some unstable results. This low value ensures more stables results use it in seconds
...
(rmg0, I) = read_esri_ascii(rainfallIntensityDEM)       # I is rainfall intensity in mm/hr
...
""" Plots and save the initial DEM """
...
while elapsed_time < model_run_time:
    ...
    if (storeData <=0) or (storeNow==1):
        ...
        storeData = round(storeDataOrg-of.dt, dtPrecision)
        storeNow=0
    ...
    if tPlot <= 0  or (plotNow==1):
        ...
        imshow_grid(rmg, 'surface_water__depth',cmap='Blues',vmin=0,vmax=0.2,plot_name=plot_name)

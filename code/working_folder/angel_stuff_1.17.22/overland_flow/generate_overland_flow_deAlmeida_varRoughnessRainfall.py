...
a) Rainfall intensity and Manning's n now are input as raster files or landLab grids.
This change allows to include spatially variable rainfall and roughness.

b) discharge_mapper function now includes flows leaving a cell. Originally only flows
from links flowin onto a given node were included when mapping discharge. By doing this
discharge was always positive. However, this violates mass conservation when we 
calculate velocity at a node. Now the results is a vector with discharge flowing in the
...
class OverlandFlowVarRoughnessRainfall(Component):
    ...
    **Additional References**
    ...
    _name = "OverlandFlowVarRoughnessRainfall"
    ...
    _info = {
        ...
        "topographic__roughness": {
            "dtype": float,
            "intent": "in",
            "optional": False,
            "units": "-",
            "mapping": "link",
            "doc": "Land surface topographic roughness",
        },
    }
    ...
    def __init__(
        ...
        # Assiging a class variable to the rainfall intensity field.
        self._rainfall_intensity = self._grid.at_node["rainfall_intensity"]
                
        # Assiging a class variable to the roughness field.
        self._mannings_n = self._grid.at_link["topographic__roughness"]
    ...
    def overland_flow(self, dt=None):
        ...
        while local_elapsed_time < dt:
            ...
            self._rainfall_intensity = self._grid["node"]["rainfall_intensity"]
            self._q = self._grid["link"]["surface_water__discharge"]
            self._h_links = self._grid["link"]["surface_water__depth"]
            self._mannings_n = self._grid["link"]["topographic__roughness"]
    ...
    def discharge_mapper(self, input_discharge):
        """Maps discharge value from links onto nodes.

        This method takes the discharge values on links and determines the
        discharge flowing INTO a given node. These methods only operates for square 
        RasterModelGrid instances.

        Returns two numpy arrays (qx, qy)
        """

        q_vals = np.zeros(self._grid.number_of_links)
        q_vals[:] = input_discharge[:]

        q_vals *= self._grid.dx

        q_vals = (q_vals[self._grid.links_at_node] * self._grid.link_dirs_at_node)
        
        qx = q_vals[:,[0,2]].sum(axis=1)
        qy = q_vals[:,[1,3]].sum(axis=1)

...
class BedloadTransport(Component):
    ...
    _info = {
        ...
        "units": "-",
        "mapping": "node",
        "doc": "Accounts for a grain size distribution that varies in space. Each value corresponds to a given GSD.",
        },
    }
    ...
    def __init__(
        self,
        grid,
        gsd,                    # Initialize the gsd array. Must be input
        ...
        gsd :   Array float, mandatory, 
                Grain size distribution. Must contain as many GSD as different indexes are in GSDLocation 
        g :     float, optional
        ...
        self._po = np.array([0.6684,0.7639,0.8601,0.9096,0.9615,1,1.055,1.108,1.197,\
        ...
        self._oo = np.array([1.011,1.011,1.01,1.008,1.004,0.9997,0.9903,0.9789,0.9567,\
        ...
        self._so = np.array([0.8157,0.8157,0.8182,0.8233,0.8333,0.8439,0.8621,0.8825,\
    ...
    def gsdProperties(self):
              
        grainSize = self._gsd[:,0] 
        D = np.zeros(grainSize.shape[0]-1)  # One for each equivalent grain size
        f = np.zeros([grainSize.shape[0]-1,self._gsd.shape[1]-1]) # One for each equivalent grain size at each different GSD
        geoMean_nodes = np.zeros_like(self._gsd_location)
        geoStd_nodes = np.zeros_like(self._gsd_location)
        
        for i in np.arange(0,self._gsd.shape[1]-1):

            grainPerc = self._gsd[:,i+1]/100  # Now working in frequency  
    ...
    def strainFunctions(self):
        ...
        if I.shape[0]>0:
            omega0[I] = np.interp(self._phi_sgom, self._po, self._oo)[I]
            sigma0[I] = np.interp(self._phi_sgom, self._po, self._oo)[I]
    ...
    def vector_mapper(self, input_vector):
        """Maps any vector values from links onto nodes preserving the components.

        This method takes the vector values on links and determines the vector components in nodes.
        Use it for example with velocity or shear stress
        """
        vector_links = np.zeros(self._grid.number_of_links)
        vector_links[:] = input_vector[:]
        
        vector_x_right = vector_links[self._grid.links_at_node[:,2]] / self._h_links[self._grid.links_at_node[:,2]]
        vector_x_left =  vector_links[self._grid.links_at_node[:,0]] / self._h_links[self._grid.links_at_node[:,0]]
        vector_x = 0.5*(vector_x_right + vector_x_left)
        
        vector_y_upper = vector_links[self._grid.links_at_node[:,1]] / self._h_links[self._grid.links_at_node[:,1]]
        vector_y_lower = vector_links[self._grid.links_at_node[:,3]] / self._h_links[self._grid.links_at_node[:,3]]
        vector_y = 0.5*(vector_y_upper + vector_y_lower)
        
        vector = np.transpose(np.vstack((vector_x,vector_y)))
        
        self._vector = vector
    
    def bedload_mapper(self, input_bedload):
        """Maps volumetric bedload per unit width values from links (m2/s) onto nodes (m3/s).

        This method takes the discharge values on links and determines the net discharge 
        flowing through a given node. 
        """
        qb_links = np.zeros(self._grid.number_of_links)
        qb_links[:] = input_bedload[:]
        
        qbx = ( qb_links[self._grid.links_at_node[:,[0,2]]] * self._grid.link_dirs_at_node[:,[0,2]] ) * self._grid.dy
        qby = ( qb_links[self._grid.links_at_node[:,[1,3]]] * self._grid.link_dirs_at_node[:,[1,3]] ) * self._grid.dx
               
        qb_nodes = np.transpose(np.vstack((qbx[:,0],qby[:,0],qbx[:,1],qby[:,1])))        
        qb_nodes = qb_nodes.sum(axis=1)
        
        self._qb_nodes = qb_nodes